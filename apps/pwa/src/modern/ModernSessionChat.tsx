...
export function ModernSessionChat({ staff = false, requestId, supportCode }: Props): React.ReactElement {
  ...
  React.useEffect(() => {
    if (!id || (!staff && !token)) { navigate(staff ? '/m/staff' : '/m/request'); return; }
    let live: PeerSessionConnection | null = null;
    void (async () => {
      try {
        const response = await fetch(`/api/peer-support/session?requestId=${encodeURIComponent(id)}${token ? `&token=${encodeURIComponent(token)}` : ''}`, staffToken ? { headers: { Authorization: `Bearer ${staffToken}` } } : undefined);
        const data = await response.json() as { ablyChannelName?: string; channelName?: string; publicSupportCode?: string };
        ...
        live = await connectPeerSession({ requestId: id, channelName, displayName: staff ? 'Peer Support Staff' : 'You', sessionToken: token, staffToken, onMessage: (m) => setMessages((old) => old.some((x) => x.id === m.id) ? old : [...old, m]), onTyping: (p: PeerTypingPayload) => { if (p.from !== (staff ? 'Peer Support Staff' : 'You')) setTyping(p.typing); }, onCall: (payload) => { const state = payload.state; if (state === 'requested') setCall('ringing'); if (state === 'ended') { leaveLiveKitAudio(); setCall('ended'); } }, onConnectionState: (state) => { if (state === 'failed') setError('Chat connection failed.'); } });
        setConnection(live);
      } catch (cause) { setError(cause instanceof Error ? cause.message : 'Could not connect to chat.'); }
    })();
    return () => live?.close();
  }, [id, navigate, staff, staffToken, token]);
  const send = async (): Promise<void> => { if (!connection || !draft.trim()) return; const sent = await connection.publish(draft); if (sent) setMessages((old) => old.some((m) => m.id === sent.id) ? old : [...old, sent]); setDraft(''); void connection.publishTyping(false); };
  const requestCall = async (): Promise<void> => { setCall('requested'); await connection?.publishCall({ state: 'requested' }); };
  const acceptCall = async (): Promise<void> => { setCall('joining'); try { const r = await fetch('/api/livekit-token', { method: 'POST', headers: { 'Content-Type': 'application/json', ...(staffToken ? { Authorization: `Bearer ${staffToken}` } : {}) }, body: JSON.stringify({ requestId: id, token }) }); const data = await r.json() as { url?: string; token?: string }; if (!r.ok || !data.url || !data.token) throw new Error('Voice is unavailable.'); await joinLiveKitAudio({ url: data.url, token: data.token }); setCall('active'); await connection?.publishCall({ state: 'active' }); } catch (e) { setError(e instanceof Error ? e.message : 'Voice call failed.'); setCall('failed'); } };
